...
def recommended():
    columnname = ["original_title", "poster_link", "release_date", "weighted_rating", "runtime"]
    global liked_movies
    recommendedmovies = pd.DataFrame(columns = columnname)
    recommendedmovieslist = []
    for i in liked_movies:
        output = get_recommendations(i["original_title"])
        recommendedmovies = recommendedmovies.append(output)
    for j, row in recommendedmovies.iterrows():
        mdata = {
            "original_title" : row["original_title"],
            "poster_link": row["poster_link"],
            "release_date": row["release_date"],
        ...
        recommendedmovieslist.append(mdata)
    return jsonify({
